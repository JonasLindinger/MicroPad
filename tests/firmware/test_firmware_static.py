...
class FirmwareSleepContractTest(FirmwareStaticContractTest):
    ...
    def test_pre_sleep_wake_input_configuration(self):
        ...
        self.assertIn("pinMode(ENCODER_A_PIN, INPUT_PULLUP)", body)
        self.assertIn("pinMode(ENCODER_B_PIN, INPUT_PULLUP)", body)
        # ext1 any-low wake mask covers GPIO21/14/13/12/10/11 (all four column
        # pins plus the two encoder pins), derived per pin, never a stale
        # literal.
        self.assertIn("esp_sleep_enable_ext1_wakeup(", body)
        self.assertIn("ESP_EXT1_WAKEUP_ANY_LOW", body)
        self.assertIn("1ULL << COL_PINS[col]", body)
        self.assertIn("1ULL << ENCODER_A_PIN", body)
        self.assertIn("1ULL << ENCODER_B_PIN", body)
        self.assertNotIn("delay(", body)
        self.assertNotIn("Serial", body)
    ...
    def test_wake_restore_order_mqtt_and_brake(self):
        ...
        # Debounce re-prime and the preserved encoder phase precede the
        # immediate wake input processing.
        self.assertIn("primeForWake(", restore)
        self.assertIn("encoderDecoder.update(", restore)
...
class FirmwareLoopContractTest(FirmwareStaticContractTest):
    ...
    def test_loop_exact_cooperative_order(self):
        ...
        tokens = (
            ...
            "if (sleepGateReady(nowMs)) enterLightSleep(nowMs)",
        )
        pos = -1
        for token in tokens:
            at = body.index(token)
