...
- runs a fixed, in-code validation `test -s <temp>` — no user-supplied command;
- atomically `mv`s it into place, then applies the fixed reload for the
  `settings.ssh_reload_strategy` enum (`core_restart` → `ha core restart`, or
  `none` to skip);
- rejects placeholders, non-absolute remote paths, and any shell-metacharacter
  path values. No free-form remote command is accepted from the API, so a
  compromised config endpoint can never inject a remote shell command.
...
- **SSH upload:** the deployed file replaces `settings.remote_path`
  ...
  backup back to `remote_path` and re-apply the fixed reload
  (`ssh_reload_strategy: core_restart` → `ha core restart`).
